...
def grid_lines(gray: np.ndarray) -> list[int]:
    ...
    # 抑制正文笔画造成的短投影峰。
    candidates = np.where(projection >= (bottom - top) * 0.24)[0]
    ...
    raw = [int(round(np.mean(group))) for group in groups]
    if len(raw) < 4:
...
def propose(gray: np.ndarray, seeds: list[tuple[float, list[float]]] | None = None) -> list[dict]:
    ...
    if row_centers:
        for column, (left, right) in enumerate(zip(lines, lines[1:])):
            ...
            for row, row_center in enumerate(centers_for_column(seed_for(center))):
                ...
                fixed_keys.add((column, row))
    for column, (left, right) in enumerate(zip(lines, lines[1:])):
        if len(run_map[column]) < 2:
            continue
        center = (left + right) / 2
        if not is_seed_column(center):
            continue
        ...
        for row, (y1, y2) in enumerate(run_map[column]):
            if (column, row) in fixed_keys:
                continue
            # 包含完整笔势，但不吞进相邻字的留白。
...
def process(work_dir: Path, image_path: Path, *, apply: bool, replace: bool) -> tuple[int, int]:
    ...
    boxes = propose(gray, seed_columns(existing))
...
def main() -> None:
    ...
    for image in sorted(args.work_dir.glob("fatie-*.webp")):
        ...
        if not args.start <= number <= args.end:
            continue
        try:
            found, existing = process(args.work_dir, image, apply=args.apply, replace=args.replace)
